...
Nodes aka "entities" in the graph are embedded as points in latent space and then 
operators are learned to encode the "relations" linking pairs of head-tail nodes. 

This code is modified from train_ops.py (ring task) to KGE:
...
The embedding table is a typical linear mapping.
We could introduce nonlinearity through a projection map into another latent space.
Spoiler, this typically doesn't help. So most of the results are done without any 
projector, or if you like, with a projector as purely an identity operation.

In our formulation, SIGReg on the entity cloud replaces negative sampling as the 
anti-collapse force.
...
In English: Metrics are based on ranking of nearest neighbors according to a cosine metric. 
(filtered based on known positives).
"""
import argparse
import math
import os
...
from hoplas.kge import KGEModel, evaluate
from hoplas.ops import algebra_tensors, algebra_metrics
...
# KGEModel and evaluate() now live in hoplas/kge.py (imported above), shared with eval_kge.py.
...
def train(args):
    ...
    # LR schedule choices: 
    # onecycle steps per-batch (cosine up-then-down to/from --max-lr);
    ...
    def save_ckpt(epoch, metrics, tag="final"):
        blob = {"epoch": epoch, "tag": tag, "args": vars(args), "metrics": metrics,
                ...
                "algebra": algebra_tensors(model.ops),  # (Nr, n, n, n) learned algebras, or None
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for chunk in loader:
            ...
            # recon: the inverse projector must invert the projector 
            ...
            # Sigreg applies to either the raw embedding table if there's no projector or else the projected space.
            ...
            # Optional in-batch contrastive term (cosine InfoNCE)
            neg = InfoNCE(pred, t_emb, args.neg_temp) if args.lambda_neg > 0 else pred.new_zeros(())
            # Optional explicit inverse-consistency term: for learning inverse relations (off by default).
        ...
        log.update(algebra_metrics(model.ops))  # ||a - a_quat|| etc. (empty dict for non-ph/quat ops)
    ...
    final.update(algebra_metrics(model.ops))
